...
- **Activity-bar view** — A dedicated `AL EventLens` view drills four levels: source app (friendly `Name` + vendor from `NavxManifest.xml`, GUID fallback when missing) → AL object kind → object → event. Intermediate rows show aggregate event counts; every row has a codicon so the level reads at a glance.
