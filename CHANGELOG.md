...
- Nested-namespaces `SymbolReference.json` schema parser (`src/symbols/schemaNamespaces.ts`) handles the BC24+ schema where object-kind arrays live recursively inside `Namespaces[]` entries. Walks both root-level arrays and every nested namespace at any depth. Reuses `extractPublishersFromContainer` from `schemaFlat.ts` so the per-object/per-method walk isn't duplicated. With this, `detect.ts` routes both schemas correctly: `Namespaces` key present → nested parser, otherwise → flat parser. The full `.app` ingestion pipeline (`readApp` → `parseSymbolReference`) is now operational; the indexer just needs to wire it into a workspace scan.
