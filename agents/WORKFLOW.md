...
- **Layer discipline:** `src/al/`, `src/symbols/`, and `src/index/` use `vscode` only for data and platform primitives — `Uri`, `Range`/`Position`, `EventEmitter`/`Disposable`, `workspace.fs`, and `workspace` watchers. The **user-facing** surfaces — `vscode.window`, `vscode.commands`, webviews — belong to `src/extension.ts` and `src/ui/`. **One documented exception:** `src/index/reindex.ts` owns the whole activation / Refresh / folder-change index policy, including its `vscode.window.withProgress` reporter and its `vscode.window.showErrorMessage` parser-bug toast. That policy lives there rather than in `extension.ts` because `npm run compile` has esbuild overwrite `dist/extension.js` with a self-contained bundle, so anything exported from `extension.ts` is unreachable from the test host — the concurrency policy would be untestable anywhere else (issue #181). No other file under those three directories may touch `vscode.window` or `vscode.commands`; `src/index/store.ts` is **not** a second exception (it imports `vscode` only for `EventEmitter`/`Disposable`).
